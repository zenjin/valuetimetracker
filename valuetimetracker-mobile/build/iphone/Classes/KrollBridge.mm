...
@implementation ValueTimeTrackerObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_valuetimetracker);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any ValueTimeTracker patches in JS before a context is loaded... nice for 
...
{
	// create ValueTimeTracker global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_valuetimetracker = [[ValueTimeTrackerObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_valuetimetracker];
	
	NSString *_valuetimetrackerNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_valuetimetrackerNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_valuetimetracker valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.valuetimetracker.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
